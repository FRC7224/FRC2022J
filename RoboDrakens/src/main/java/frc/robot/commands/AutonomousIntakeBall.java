 ...
 *
 */
public class AutonomousIntakeBall extends CommandBase {

    private final IntakeSubsystem m_intakesubsystem;
    private final Timer timer = new Timer();
    boolean ballLoaded = false;

    public AutonomousIntakeBall(IntakeSubsystem subsystem) {
    ...
    public void execute() {
        ...
        if (timer.get() <= timetorun) {
            if (m_intakesubsystem.getballLoadstatus()) { // is ball loaded ?
                ...
                m_intakesubsystem.setIntakeMotor(Constants.kIntakeSpeed);
