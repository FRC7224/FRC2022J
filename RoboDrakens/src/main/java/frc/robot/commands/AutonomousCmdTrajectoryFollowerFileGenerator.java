...
public class AutonomousCmdTrajectoryFollowerFileGenerator extends CommandBase {
  ...
  protected void initialize() {
    ...
    left.configureEncoder(m_chassis.getLeftEncoderPosition(), 365, 0.1524);
    right.configureEncoder(m_chassis.getRightEncoderPosition(), 365, 0.1525);
    ...
    t.schedule(new TimerTask() {
      ...
      public void run() {
        ...
        if (!left.isFinished() || !right.isFinished()) {
          ...
          double l = left.calculate(m_chassis.getLeftEncoderPosition());
          double r = right.calculate(m_chassis.getRightEncoderPosition());
          double gyro_heading = -m_chassis.getGyroAngle(); // Assuming the gyro is giving a value in degrees
          double desired_heading = Pathfinder.r2d(left.getHeading()); // Should also be in degrees
          double angleDifference = Pathfinder.boundHalfDegrees(desired_heading - gyro_heading);
          double turn = 0.8 * (-1.0 / 80.0) * angleDifference;
          m_chassis.tankDrive((l + turn), (r - turn));
          /*
